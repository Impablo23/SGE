...
export class AlumnosComponent implements OnInit {
  ...
  async addContacto(idEntidad: number) {
    const dialogRef = this.dialog.open(AddAlumnoComponent, { data: idEntidad, scrollStrategy: this.overlay.scrollStrategies.noop() });
  ...
  async editContacto(contacto: Contacto) {
    const dialogRef = this.dialog.open(EditAlumnoComponent, { data: contacto, scrollStrategy: this.overlay.scrollStrategies.noop() });
    const RESULT = await dialogRef.afterClosed().toPromise();
    if (RESULT) {
      if (RESULT.ok) {
        this.ngOnInit();
  ...
  async deleteContacto(contacto: Contacto) {
    const dialogRef = this.dialog.open(DeleteAlumnoComponent, { data: contacto, scrollStrategy: this.overlay.scrollStrategies.noop() });
    const RESULT = await dialogRef.afterClosed().toPromise();
    if (RESULT) {
      if (RESULT.ok) {
        this.ngOnInit();
